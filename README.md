...
![Démo vidéo](placeholder_video_demo.gif)
...
|---|---|
| ![Capture application](placeholder_screenshot_app.png) | ![Capture base de données](placeholder_screenshot_db.png) |
...
2. Placer le dossier `geotracker/` dans `htdocs/`
3. Exécuter le script SQL dans phpMyAdmin pour créer la base et la table
4. Tester avec Postman → `POST http://localhost/geotracker/saveGpsPoint.php`

### Côté Android
1. Ouvrir le projet dans **Android Studio**
2. Définir l'URL du serveur dans `MainActivity.java` :
   - Émulateur → `http://10.0.2.2/geotracker/saveGpsPoint.php`
   - Vrai téléphone → `http://VOTRE_IP_LOCALE/geotracker/saveGpsPoint.php`
3. Ajouter dans `AndroidManifest.xml` : `android:usesCleartextTraffic="true"`
4. Compiler et lancer — accepter les permissions de localisation au premier démarrage
...
- L'émulateur n'a pas de GPS réel — utiliser **Commandes étendues → Localisation** pour simuler une position
- Le téléphone et le serveur doivent être sur le même réseau local lors de l'utilisation d'un vrai appareil
